...
{
    public class DatosObjetosWS : GestionBD
    {
        const string obtenDatosBD = "Sp_ObtenDatosWS";
        ...
        public bool ObjetosWSConsulta(int Tipo, int UsuarioID, int AplicacionID)
        {
            ...
            try
            {
                ...
                if (Lector.Read())
                {
                    ...
                    XmlNode xmlNode = resultadoXML.DocumentElement.SelectSingleNode("DatosWS");
                    respuesta = xmlNode.HasChildNodes;
                }
                CerrarConexion();
                if (respuesta)
                    EscribeLog("Correcto: Usuario: " + UsuarioID + " Consulto Objetos WS");
                else
                    EscribeLog("Error: Usuario: " + UsuarioID + " No Consulto Objetos WS");
            }
            catch (Exception Err)
            {
                EscribeLog("Excepcion: DatosObjetosWS.ObjetosWSConsulta" + Err.Message.ToString());
            }
            return respuesta;
        }
    }
